...
const compiler = async (text: string) => {
  ...
  const traverse = (parent: Node) => {
    ...
    if (is.tag(parent) && parent.name === "script") {
      ...
      if (scriptSrc) {
        const isExternalScript = scriptSrc.startsWith("http") || scriptSrc.startsWith("/") // a script starting with `/` is considered external, in the sense that it's fetched over the network at runtime. However Astro does support you putting scripts into a `public` dir and then consuming them at `/`, so you could argue that's an import. The risk of false positives is high, though, since scripts other than those at `public/` might be available at `/`.
        if (!isExternalScript) {
