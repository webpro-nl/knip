...
const resolveConfigWithNuxt = async (options: PluginOptions) => {
  ...
  nuxt.hook('imports:extend', imports => {
    for (const i of imports) {
      if (isAbsolute(i.from)) availableSources.add(i.from);
  ...
  nuxt.hook('components:extend', components => {
    for (const c of components) {
      if (isAbsolute(c.filePath)) availableSources.add(c.filePath);
    }
  });
  ...
  const pagesPatterns =
    typeof opts.pages === 'boolean' || !opts.pages.pattern ? [`**/*{${extensionGlob}}`] : toArray(opts.pages.pattern);
  ...
  for (const layer of opts._layers) {
    ...
    if (isNitroImportsEnabled) {
      availableSources.add(resolve(rootDir, cfg.dir?.shared ?? 'shared', `utils/*{${extensionGlob}}`));
      availableSources.add(resolve(rootDir, cfg.dir?.shared ?? 'shared', `types/*{${extensionGlob}}`));
    }

    inputs.push(...entryPatterns.map(s => toProductionEntry(s)));
  }

  for (const path of availableSources) inputs.push(toProductionEntry(path, { allowIncludeExports: true }));
