...
import { default as postcss } from '../../src/plugins/postcss/index.js';
...
test('Find dependencies in PostCSS configuration (package.json)', async () => {
  const dependencies = await postcss.findDependencies(manifestFilePath, options);
...
test('Find dependencies in PostCSS configuration (postcss.config.cjs)', async () => {
  ...
  const dependencies = await postcss.findDependencies(configFilePath, options);
