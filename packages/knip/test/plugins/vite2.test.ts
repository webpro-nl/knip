...
test('Find extension issues with incomplete config', async () => {
  ...
  const fileIssues = Array.from(issues.files).filter(issue => 
    issue && (
      issue.includes('mock.ts') || 
      issue.includes('mock.desktop.ts')
    )
  );
  
  assert.ok(fileIssues.length > 0, 'Should find issues with mock files');
  
  console.log('Issues with incomplete config:', JSON.stringify(fileIssues, null, 2));
});
