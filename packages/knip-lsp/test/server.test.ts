...
describe("Knip LSP Server", () => {
  ...
  describe("Diagnostics", () => {
    test("should generate diagnostics for Knip issues", async () => {
      ...
      const indexUri = fileUri(join(fixtureDir, "index.ts"));
    ...
    test("should detect unused dependencies", async () => {
      ...
      for (const [_uri, data] of diagnostics) {
        for (const diagnostic of data.diagnostics) {
          if (
            diagnostic.code === "dependencies" &&
    ...
    test("should detect unused exports", async () => {
      ...
      const unusedExport = indexDiagnostics?.diagnostics.find(
        (d: any) =>
          d.code === "exports" && d.message.includes("unusedFunction"),
      );
    ...
    test.skip("should detect unused class members", async () => {
      // Skipping: Class member detection requires specific Knip configuration
      await client.initialize(rootUri);
      const diagnostics = await client.collectDiagnostics(10000);
      ...
      const unusedClassMember = indexDiagnostics?.diagnostics.find(
        (d: any) =>
          d.code === "classMembers" && d.message.includes("unusedMethod"),
      );
  ...
  describe("Code Actions", () => {
    test("should provide code actions for diagnostics", async () => {
      ...
      const indexUri = fileUri(indexPath);
      const fs = await import("node:fs/promises");
      const indexContent = await fs.readFile(indexPath, "utf-8");
      await client.openTextDocument(indexUri, indexContent, "typescript");
      ...
      if (indexDiagnostics && indexDiagnostics.diagnostics.length > 0) {
        ...
        const codeActions = await client.sendRequest(
        ...
        const fixAction = codeActions.find(
          (a: any) => a.kind === "quickfix" && a.command,
        );
  ...
  describe("Commands", () => {
    test("should execute knip.analyze command", async () => {
      ...
      // Should complete without error (LSP returns null for void commands)
      expect(result).toBeNull();
    }, 2000);
  });

  describe("Configuration", () => {
    test.skip("should respect configuration settings", async () => {
      // Skipping: Configuration changes require server support for
      // workspace/configuration requests which needs more implementation
      await client.initialize(rootUri);
