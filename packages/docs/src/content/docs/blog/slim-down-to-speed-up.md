...
bottleneck had been eliminated and Knip is going to be a lot faster. Skip
straight to the bottom to install v4 canary, or grab yourself a nice drink and
read on if you're interested in where we are coming from, and where we are
heading.
...
like module resolution and accurately finding references to exported values. It
also allows to customize parts of it, such as the (virtual) file system or the
module resolver.
...
`ts.createProgram()` method is used to create a program and this will read and
parse source code files, resolve modules, and so on.
...
Fortunately, everything that's imported and exported (including attached members
for namespaces or enums) can be found relatively easily during AST traversal, so
references to exports don't have to be "traced back" later on.
...
memory or on disk). Taking class members out of the equation simplifies things a
lot and paves the way for all sorts of improvements.
...
- The serialization format is still being explored and there is no caching yet,
  but having the steps more decoupled is a Good Thing™️.
